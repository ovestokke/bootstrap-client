...
# @version: 2.0
#
# System configuration, bloatware removal, WSL, and application installation
...
# Start logging
$logFile = Join-Path $PSScriptRoot "Setup-System-Log-$(Get-Date -Format 'yyyy-MM-dd-HHmmss').txt"
...
$wslInstalled = $false
try {
    $wslOutput = wsl.exe --list 2>&1
    if ($LASTEXITCODE -eq 0) {
        $wslInstalled = $true
        Write-Host "[OK] WSL is already installed" -ForegroundColor Green

        # Remove null characters from UTF-16 output
        $wslCheck = $wslOutput -replace '\x00', '' -replace '\r', ''
...
Write-Host "  [1] Skip       - System setup only (no apps)" -ForegroundColor White
Write-Host "  [2] Basic      - Essential apps only (~10 apps)" -ForegroundColor White
Write-Host "  [3] Gaming     - Gaming platforms + peripherals + monitoring" -ForegroundColor White
Write-Host "  [4] Developer  - Development tools only" -ForegroundColor White
